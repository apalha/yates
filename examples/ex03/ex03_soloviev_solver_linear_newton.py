# -*- coding: utf-8 -*-
""" Test of the nonlinear solver for the Grad-Shafranov equation (Picard or Newton solver).

Description
-----------
This script tests the implementation of the GS module for solving the Grad-Shafranov equation. This test solves a nonlinear
Grad-Shafranov equation, see [1]_ and [2]_. The boundary conditions used
...
def J(r,z,psi):
    return r*r*0.95 + 0.05 + psi*psi + psi*psi*psi #1.0 + psi + 0.1*psi*psi + 0.1*psi*psi*psi #numpy.sin(psi*psi)+1

def dJ(r,z,psi):
    return 2.0*psi + 2.0*psi*psi #1.0 + psi + 0.1*psi*psi + 0.1*psi*psi*psi #numpy.sin(psi*psi)+1
...
for k in range(10):
    ...
    plasma.solve_step(J,dJ)#,bc_function=psi_python)
